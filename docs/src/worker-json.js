    ...
    (function() {
        ...
        this.emit = function(name, data) {
            postMessage({
                type: "event",
                name: name, // "annotate"
                data: data  // array of error objects
            });
        };
...
window.onmessage = function(e) {
    var msg = e.data;
    if (msg.event && sender) {
        sender._signal(msg.event, msg.data); //msg.data: [{start}, [inserted text]]
    }
    else if (msg.command) {
        if (main[msg.command]) {
            main[msg.command].apply(main, msg.args);
            if (msg.command == 'setDataModel') {
                main['onUpdate'].call(main);
            }
        }
        ...
        window.initBaseUrls(msg.tlns);//copy properties from msg.tlns to require.tlns{}
        require("ace/lib/es5-shim");
        sender = window.sender = window.initSender();//Sender()
        //msg.module: "ace/mode/json_worker"
        //msg.classname: "JsonWorker"
        var clazz = require(msg.module)[msg.classname];//exports.JsonWorker()
...
"use strict";
    ...
    var at,     // The index of the current character
        ...
        next = function (c) {
            ...
            if (c && c !== ch) {
                error("Syntax error: Expected '" + c + "' instead of '" + ch + "'");
            }
        ...
        white = function () {
            // current char is not null and must be 'readable' symbols
            while (ch && ch <= ' ') {
                next();
            }
        },
...
"use strict";
    ...
    function validateResourcesValues (className, str) {
        ...
        var el, subEl,
            classObj, attrObj;
        for (el in classList) {
            classObj = classList[el];
            if (classObj.class == className) {
                for (subEl in classObj.attributes) {
                    attrObj = classObj.attributes[subEl];
                    if (attrObj.name == str) {
                        return true;
                    }
                }
